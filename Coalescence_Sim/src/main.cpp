...
void roli_comp() {
	vector<double> mean_list = {10.0};
	vector<double> peffect_list = {0.95};
	vector<double> pcluster_list = {0.15};
	vector<int> div_list = {3};
...
void run_comp(config::simulation_pars pars, TFile *out_file) {
	...
	vector<double> ratios = ratios_map_to_vec(data.ratios[pars.divisions][0]);
	TH1D *dylan = hist_ratios(ratios);
	TH1D *roli = sim_efficiency(pars.p_effect, pars.p_cluster, pars.particle_mean, pars.divisions, pars.n_events, out_file);
