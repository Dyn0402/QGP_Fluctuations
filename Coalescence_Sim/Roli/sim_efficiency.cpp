...
TH1D* sim_efficiency(double pc, double xcluster, double mean, int divs, int events) {
	...
	int nParticle, nParticle1, phibin, nglob;
//	int check=0, check1=0, a, c;

	double xevent, xphi, phi, fm;
//	double efficiency, x;
	double pratio;
...
//    TH1F *hmixed[max_nPhiBin];
    hratio[0] = new TH1F("Roli", "Roli", 23, -0.05, 1.10);
	for(int i = 1; i < nPhiBin; i++){
		sprintf(name,"hratio_Phi_%d",i);
		hratio[i] = new TH1F(name,"",23,-0.05,1.10);
