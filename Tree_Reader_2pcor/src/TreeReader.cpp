...
void TreeReader::set_2pcor_bins(int bins, double x_low, double x_high) {
...
void TreeReader::read_trees() {
	define_qa();
	mix.set_hist_bins(hist_nbins, hist_xlow, hist_xhigh);
...
void TreeReader::process_event(Event& event) {
	...
	if(check_event(event)) {
		...
		int nparticles = good_particle_angles.size();
		if(nparticles >= cut.min_multi) {
			...
			post_n_particles[cent].Fill(nparticles);
			...
			if(rand_data) { random.append_event(nparticles, event.get_refn(), trand); }
...
//			}
			...
			vector<double> event_2pcorr = get_two_particle_correlation(good_particle_angles);  // Convert proton angles in event to 2pcor values.

			// Save 2pcor values to data
			if(cbwc) {
				if((string)data[event.get_refn()][nparticles].GetName() == "") {
					string name_title = "Twopcorr_Raw_Cent_" + to_string(cent) + "_NParticles_" + to_string(nparticles);
					data[event.get_refn()][nparticles].SetNameTitle(name_title.data(), name_title.data());
					data[event.get_refn()][nparticles].SetBins(hist_nbins, hist_xlow, hist_xhigh);
				}
				for(auto &xi:event_2pcorr) {
					data[event.get_refn()][nparticles].Fill(xi);
				}
			} else {
...
void TreeReader::write_2pcor_hists() {
	TFile twopcor((qa_path + twopcorr_out_name + to_string(energy) + "GeV.root").data(), "RECREATE");
	for(auto &cent:data) {
		for(auto &particles:cent.second) {
			particles.second.Write();
		}
	}
	if(mixed) {
		for(auto &cent:mix.get_data()) {
			for(auto &particles:cent.second) {
				particles.second.Write();
			}
		}
