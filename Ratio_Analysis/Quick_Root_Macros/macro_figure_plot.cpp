...
#include "figure_plot_helpers.h"

using namespace std;


// Forward declaration of functions from figure_plot_helpers.h
...
// Forward declarations of functions defined in this file (which reference the globals defined below)
...
void read_systematics(string path);


// Class variables which are written as globals for macro
...
string out_path = "/home/dylan/Research/Results/Azimuth_Analysis/";  // Directory to output root file with figures
string out_root_name = "8-4-21_macro_test.root";  // Name of output root file with figures
string in_file_path = "/home/dylan/Research/Results/Azimuth_Analysis/sys_vals_8-3-21_write_test.txt";  // Path to input file with stat values
bool zoom = false;



void macro_figure_plot() {
	read_systematics(in_file_path);
	plot_paper_figs();
	cout << "donzo" << endl;
}
...
//	can->SetPad(0, 0.01, 0.99, 1);
	DividegPad((int)bes1["raw"].size(), 2, 0.1, 0.02, 0.02, 0.1);
...
//	can->Divide(3, 3, 0, 0);
	DividegPad(3, 3, 0.1, 0.02, 0.02, 0.1);
	int can_index = 0;
	...
	for (pair<float, plot_data> rapid : bes1["raw"]) {  // Iterate over rapidities, left to right on plot
		...
		TText* text = new TText(5, 1.015, ("|#eta|<" + to_string(rapid.first).substr(0, 3)).data());
		text->SetTextSize(10); text->Draw();
