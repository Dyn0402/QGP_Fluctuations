...
//
//// Calculate statistics for total_proton slice histogram data.
//void BinomialAnalyzer::calc_stat(map<double, int> &slice_data, int protons, int energy, int div, int cent, map<int, map<int, map<int, map<int, map<string, Measure>>>>> *stats) {
//	Stats stat(slice_data);
////	cout << "Getting energy " << to_string(energy) << " div " << to_string(div) << " cent " << to_string(cent) << " proton " << to_string(protons) << endl;
//	(*stats)[energy][div][cent][protons]["mean"] = stat.get_mean();
//	(*stats)[energy][div][cent][protons]["standard_deviation"] = stat.get_standard_deviation();
...
//				}
//				for(auto &total_proton:data[energy][div][cent].get_diff_slice_hist()) {
//					slice_diff_dist_plot(total_proton.second, total_proton.first, div, "Diff Slice Dist " + to_string(total_proton.first) + " protons " + to_string(energy) + "GeV " + to_string(cent) + " cent " + to_string(div) + " divs");
//				}
//			}
//		}
//	}
...
//
//
//void BinomialAnalyzer::slice_diff_dist_plot(map<double, int> &slice_data, int total_protons, int div, string name) {
//	TCanvas can((name + " canvas").data(), (name + " canvas").data(), canvas_width, canvas_height);
//	gStyle->SetOptStat("niMRSK");
//	TH1D dist((name + " hist").data(), (name + " hist").data(), total_protons+1, -(float)total_protons/div, (float)total_protons*(1 - 1.0 / div));
//	TH1D binom((name + " hist_binom").data(), (name + " binom").data(), total_protons+1, -(float)total_protons/div, (float)total_protons*(1 - 1.0 / div));
//	binom.SetLineColor(kRed);
//
//	for(auto &bin_protons:slice_data) {
//		dist.Fill(bin_protons.first, bin_protons.second);
